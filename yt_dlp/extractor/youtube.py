...
from ..downloader.utils.playback import PlaybackFinish
...
class YoutubeIE(YoutubeBaseInfoExtractor):
    ...
    def _live_dash_fragments(self, video_id, format_id, live_start_time, mpd_feed, manifestless_orig_fmt, ctx):
        ...
        mpd_url, stream_number, is_live = mpd_feed(format_id, 18000)
        if not is_live:
        ...
        requested_format = next(fmt for fmt in formats if fmt['manifest_stream_number'] == stream_number)
        fragments = requested_format['fragments']
        fragment_base_url = requested_format['fragment_base_url']
        assert fragment_base_url

        last_fragment = fragments[-1]
        last_seq = int(re.search(r'(?:/|^)sq/(\d+)', last_fragment['path']).group(1))
        first_possible_index = last_seq - int(MAX_DURATION // fragments[-1]['duration'])

        new_fragments = []
        for idx in range(first_possible_index, last_seq):
            last_segment_url = urljoin(fragment_base_url, f'sq/{idx}')
            duration = last_fragment['duration']
            start = last_fragment['start'] - (last_seq - idx) * duration
        ...
        return new_fragments + fragments
